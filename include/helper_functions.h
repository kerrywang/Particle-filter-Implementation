 ...
 */
    ...
    /**
     * find the closest landmark from the observed data
     */
     inline LandmarkObs NearestNeighbor(const std::vector<LandmarkObs>& pool, const LandmarkObs& target) {
         assert(pool.size() > 0);
         LandmarkObs closest = pool[0];
         double closest_dist = dist(closest.x, closest.y, target.x, target.y);
         for (int i = 0; i < pool.size(); i++) {
             double new_dist = dist(pool[i].x, pool[i].y, target.x, target.y);
     ...
     /**
      * Calculate Multivariabte gaussian weight
      */
    ...
    /**
     * Transform the landmark observation to the particle coordinate
     */
